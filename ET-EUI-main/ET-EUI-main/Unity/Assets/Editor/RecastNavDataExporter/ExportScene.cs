    ...
    internal class MapExport2Nav: Editor
    {
        ...
        static void ImportSceneMesh()
        {
            ...
            if (path.Length != 0)
            {
                ...
                EditorSceneManager.OpenScene("Assets/Scenes/Map3.unity");
                for (int i = 0; i < data.Scenes.Count; i++)
                {
                    ...
                    for (int j = 0; j < scene.Objects.Count; j++)
                    {
                        ...
                        GameObject obj = null;
                        string addressPath;
                        Debug.LogError("导入场景类型数：" + scene.Objects.Count);
                        Debug.LogError("导入场景类型：" + objInfo.Type);
                        switch (objInfo.Type)
                        {
                            case "Prefab":
                                addressPath = objInfo.PrefabPath;
                                ...
                                break;
                            case "Terrain":
                                addressPath = objInfo.Terrain.TerrainPath;
        ...
        static AssetsRoot GetData()
        {
            ...
            foreach (UnityEngine.Object selectObject in selectedAssetList)
            {
                ...
                foreach (GameObject sceneObject in UnityEngine.Object.FindObjectsOfType(typeof (GameObject)))
                {
                    // 如果对象是激活状态
        ...
        public static void ChangeObj2Data(GameObject sceneObject,AssetsScene root)
        {
            ...
            if (terrain != null)
            {
                ...
                string prefabObject = AssetDatabase.GetAssetPath(terrain.terrainData);
                string materialObject = AssetDatabase.GetAssetPath(t.materialTemplate);
            ...
            // 判断是否是预设
            else if (IsPrefabAsset(sceneObject))
            {
                // 获取引用预设对象
                UnityEngine.Object prefabObject = PrefabUtility.GetCorrespondingObjectFromSource(sceneObject);
        ...
        public static bool IsPrefabAsset(UnityEngine.Object obj, bool includePrefabInstance = true)
        {
#if UNITY_EDITOR
            if (!obj)
            {
